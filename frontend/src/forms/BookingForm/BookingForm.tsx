...
const BookingForm = ({ currentUser, paymentIntent }: Props) => {
  ...
  const onSubmit = async (formData: BookingFormData) => {
    ...
    if (result.error) {
      ...
      // Payment Intent requires action, handle 3D Secure authentication
      const actionResult = await stripe.handleCardAction(paymentIntent.clientSecret);
      if (actionResult.error) {
        // Handle error
        setPaymentStatus("failed");
      } else {
        // 3D Secure authentication successful, re-submit the payment
        onSubmit(formData);
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end">
        ...
        {paymentStatus === "succeeded" && <p>Payment succeeded!</p>}
